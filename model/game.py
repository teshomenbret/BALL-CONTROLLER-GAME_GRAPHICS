...
class Game:
    ...
    def addRandomBlock(self, dt):

        self.random_dt += dt
        if self.random_dt >= 900:
            runNum = random.random()
            if runNum < 0.055:
                self.random_dt = 0
                self.generateBlock(runNum)
                

            
    def generateBuilding(self, dt):
